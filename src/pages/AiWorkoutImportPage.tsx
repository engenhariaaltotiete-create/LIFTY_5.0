...
export default function AiWorkoutImportPage(){
 ...
 function next(){const error=validateCurrent();if(error)return alert(error);setStep(s=>Math.min(7,s+1))}
...
- Todo bloco de alongamento deve começar com uma única preparação de 5 segundos: use COUNTDOWN_SECONDS: 5. Essa preparação ocorre somente uma vez, no início do bloco, e não antes de cada exercício.
- Cada exercício de alongamento deve ter exatamente WORK_SECONDS: 12.
- Entre um exercício e o próximo, use REST_SECONDS: 2. O último exercício do bloco deve usar REST_SECONDS: 0, sem descanso adicional ao terminar.
- Para o bloco de alongamento, use MODEL: interval, INTERVAL_MODE: custom e ROUNDS: 1, salvo se o usuário pedir explicitamente outra configuração.
- Escolha os exercícios de alongamento de forma coerente com a modalidade, a divisão muscular e o conteúdo daquele treino.
