...
const validModels=['resistance','cardio','interval'] as const
const validIntervalModes=['tabata','emom','amrap','custom'] as const

const headerAnywhere=/===\s*LIFTY\s+WORKOUT\s*===/gi
const endAnywhere=/===\s*END\s+LIFTY\s+WORKOUT\s*===/gi
const workoutHeader=/^\s*===\s*LIFTY\s+WORKOUT\s*===\s*$/i
const workoutEnd=/^\s*===\s*END\s+LIFTY\s+WORKOUT\s*===\s*$/i
const blockMarker=/^\s*\[\s*BLOCK\s*\]\s*$/i
const exerciseMarker=/^\s*\[\s*EXERCISE\s*\]\s*$/i

const acceptedKeys=[
 'VERSION','NAME','DESCRIPTION','MODEL','INTERVAL_MODE','ROUNDS','ROUND_REST_SECONDS','TOTAL_MINUTES','COUNTDOWN_SECONDS',
 'KIND','SETS','REPS_MIN','REPS_MAX','TARGET_SECONDS','TARGET_MINUTES','TARGET_DISTANCE_KM','WORK_SECONDS','REST_SECONDS','TARGET_REPS','NOTES'
] as const

const acceptedKeysPattern=acceptedKeys.join('|')
const structuralTokenPattern=new RegExp(`(===\\s*END\\s+LIFTY\\s+WORKOUT\\s*===|===\\s*LIFTY\\s+WORKOUT\\s*===|\\[\\s*BLOCK\\s*\\]|\\[\\s*EXERCISE\\s*\\]|(?:${acceptedKeysPattern})\\s*:)`,'gi')
...
function extractWorkoutTexts(text:string){
 ...
 const headers=[...source.matchAll(headerAnywhere)]
 if(!headers.length)return []
 const parts:string[]=[]
 for(let i=0;i<headers.length;i++){
  const start=headers[i].index??0
  const nextStart=i+1<headers.length?(headers[i+1].index??source.length):source.length
  const slice=source.slice(start,nextStart)
  endAnywhere.lastIndex=0
  const endMatch=endAnywhere.exec(slice)
  const end=endMatch?(endMatch.index+endMatch[0].length):slice.length
  parts.push(slice.slice(0,end).trim())
 }
 return parts
}

function normalizeWorkoutStructure(text:string){
 const cleaned=cleanText(text)
 return cleaned
  .replace(structuralTokenPattern,'\n$1')
  .split('\n')
  .map(x=>x.trim())
  .filter(Boolean)
  .map(line=>{
   if(workoutHeader.test(line))return '=== LIFTY WORKOUT ==='
   if(workoutEnd.test(line))return '=== END LIFTY WORKOUT ==='
   if(blockMarker.test(line))return '[BLOCK]'
   if(exerciseMarker.test(line))return '[EXERCISE]'
   return line
  })
}

function parseOneWorkoutText(text:string){
 const lines=normalizeWorkoutStructure(text)
 if(!lines.length||lines[0]!=='=== LIFTY WORKOUT ===')throw new Error('Texto LIFTY inválido: cabeçalho não encontrado.')
 const sections:TextSection[]=[{type:'root',values:{}}]
 let current=sections[0]
 for(const line of lines.slice(1)){
  if(line==='=== END LIFTY WORKOUT ===')break
  if(line==='[BLOCK]'){current={type:'block',values:{}};sections.push(current);continue}
  if(line==='[EXERCISE]'){current={type:'exercise',values:{}};sections.push(current);continue}
  const i=line.indexOf(':')
  if(i<1)continue
  const key=line.slice(0,i).trim().toUpperCase()
  const value=line.slice(i+1).trim()
  if((acceptedKeys as readonly string[]).includes(key))current.values[key]=value
 }
 ...
 for(const sec of sections.slice(1)){
  if(sec.type==='block'){b={values:sec.values,exercises:[]};parsedBlocks.push(b)}
   ...
   if(!b)throw new Error('Exercício encontrado antes de um bloco.')
   b.exercises.push(sec.values)
  }
 }
...
function validateParsedWorkout(parsed:ParsedWorkout,index:number){
 ...
 for(const pb of parsed.blocks){
  pb.values.MODEL=(pb.values.MODEL||'').toLowerCase()
  if(!validModels.includes(pb.values.MODEL as typeof validModels[number]))throw new Error(`Modelo de bloco inválido no treino "${workoutName}": ${pb.values.MODEL||'(vazio)'}.`)
  ...
  if(pb.values.INTERVAL_MODE)pb.values.INTERVAL_MODE=pb.values.INTERVAL_MODE.toLowerCase()
  if(pb.values.MODEL==='interval'&&pb.values.INTERVAL_MODE&&!validIntervalModes.includes(pb.values.INTERVAL_MODE as typeof validIntervalModes[number]))throw new Error(`INTERVAL_MODE inválido no bloco "${pb.values.NAME}" do treino "${workoutName}".`)
  for(const pe of pb.exercises){
   if(!pe.NAME)throw new Error(`Exercício sem nome no bloco "${pb.values.NAME}" do treino "${workoutName}".`)
   if(pe.KIND)pe.KIND=pe.KIND.toLowerCase()
