...
function parseOneWorkoutText(text:string){
 ...
 if(lines[0]!=='=== LIFTY WORKOUT ===')throw new Error('Texto LIFTY inválido: cabeçalho não encontrado.')
...
export async function importLiftyText(text:string){
 const parts=cleanText(text).split(/(?==== LIFTY WORKOUT ===)/g).map(x=>x.trim()).filter(x=>x.startsWith('=== LIFTY WORKOUT ==='))
 if(!parts.length)throw new Error('Nenhum treino LIFTY encontrado no texto.')
 const ids:number[]=[]
 for(const part of parts){
  const parsed=parseOneWorkoutText(part);const library=await db.exercises.toArray();const now=new Date().toISOString();const blocks=[] as WorkoutTemplate['blocks']
  for(let bi=0;bi<parsed.blocks.length;bi++){
   const pb=parsed.blocks[bi],model=pb.values.MODEL as WorkoutTemplate['blocks'][number]['model']
   if(!['resistance','cardio','interval'].includes(model))throw new Error(`Modelo de bloco inválido: ${pb.values.MODEL||'(vazio)'}.`)
   if(!pb.values.NAME)throw new Error('Bloco sem nome.')
   const exercises=[] as WorkoutTemplate['blocks'][number]['exercises']
   for(let ei=0;ei<pb.exercises.length;ei++){
    const pe=pb.exercises[ei];if(!pe.NAME)throw new Error(`Exercício sem nome no bloco ${pb.values.NAME}.`)
    ...
    if(!def){const id=await db.exercises.add({name:pe.NAME,kind,system:false,loadUnit:kind==='resistance'?'kg':'none'});def={id,name:pe.NAME,kind,system:false,loadUnit:kind==='resistance'?'kg':'none'};library.push(def)}
    const n=(k:string)=>pe[k]!==undefined&&pe[k]!==''?Number(pe[k]):undefined
   ...
   if(!exercises.length)throw new Error(`O bloco ${pb.values.NAME} não possui exercícios.`)
   const n=(k:string)=>pb.values[k]!==undefined&&pb.values[k]!==''?Number(pb.values[k]):undefined
  ...
  const id=await db.workouts.add({name:parsed.root.NAME,description:parsed.root.DESCRIPTION,formatVersion:4,blocks,archived:false,createdAt:now,updatedAt:now});if(id!==undefined)ids.push(id)
 }
 return ids
}
